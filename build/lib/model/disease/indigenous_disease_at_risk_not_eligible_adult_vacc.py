...
class IndDisease(Disease):
    ...
    def __init__(self, p, cmatrix,rng, fname, mode):
        super(IndDisease, self).__init__(p, cmatrix,rng, fname, mode)
    ...
    def _load_disease_data(self,p):
        ...
        self.varying_external_exposure = p["varying_external_exposure"]
        self.external_exposure_prob_multipliers = \
                                    p["external_exposure_prob_multipliers"]
        ...
        at_risk_perc = at_risk_perc.filter(
            pl.col("Ind_status") == "Indigenous").drop("Ind_status")
        ...
        self.prev_vacc_list = pl.DataFrame(
            {"vaccine": (["atrisk_ppv23_adult"] * 96 +
             ...
             ["ppv23_pcv7_follow"] * 101 +
             ["ppv23_pcv20_follow"] * 101 +
             ["ppv23_pcv13_follow"] * 101 +
             ["ppv23_pcv20_follow_2"] * 101 +
             ["ppv23_pcv13_follow_2"] * 101 +
             ["ppv23_pcv20_adult_follow"] * 101 +
             ["ppv23_pcv13_adult_follow"] * 101 +
             ["ppv23_pcv20_adult_follow_2"] * 101 +
             ["ppv23_pcv13_adult_follow_2"] * 101 +
             ["ppv23_pcv20_adult_follow_50"] * 101 +
             ["ppv23_pcv20_adult_follow_2_50"] * 101 
             ),
             "age": list(range(5,101)) * 4 + list(range(0,101)) * 11,
             "prev_eff_vaccine": \
        ...
        ######## ######## ###### higher disease outcome ######## ##### #####
        no_strains = len(p["strains_with_higher_disease_outcome"])
        no_ages =  [cur_ages[1] - cur_ages[0] + 1  for cur_ages in  
                    p["higher_disease_outcome_age_groups"]]
        given_disease_outcome_multipliers = pl.DataFrame([ 
            pl.Series("age", list(chain(*[list(range(cur_ages[0], 
                                cur_ages[1] + 1)) for cur_ages 
                    in p["higher_disease_outcome_age_groups"]])) * no_strains),
            pl.Series("strains", list(chain(*[[x] * sum(no_ages) 
                      for x in p["strains_with_higher_disease_outcome"]]))),
                     # p["strains_with_higher_disease_outcome"] *\
                     # sum(no_ages)),
            pl.Series("multiplier", list(chain(*[[x] * y for (x,y) 
                        in zip(p["higher_disease_outcome_multipliers"], 
                               no_ages)])) *  no_strains)
        ...
        #lower disease outcome
        no_strains = len(p["strains_with_lower_disease_outcome"])
        no_ages =  [cur_ages[1] - cur_ages[0] + 1  for cur_ages in  
                    p["lower_disease_outcome_age_groups"]]
        given_lower_disease_outcome_multipliers = pl.DataFrame([ 
            pl.Series("age", list(chain(*[list(range(cur_ages[0], 
                                cur_ages[1] + 1)) for cur_ages 
                    in p["lower_disease_outcome_age_groups"]])) * no_strains),
            pl.Series("strains", list(chain(*[[x] * sum(no_ages) 
                      for x in p["strains_with_lower_disease_outcome"]]))),
                     # p["strains_with_higher_disease_outcome"] *\
                     # sum(no_ages)),
            pl.Series("multiplier", list(chain(*[[x] * y for (x,y) 
                        in zip(p["lower_disease_outcome_multipliers"], 
                               no_ages)])) *  no_strains)
            
            ]).sort("age", "strains")
        self.disease_outcome_multipliers = \
            (self.disease_outcome_multipliers.update(
            given_lower_disease_outcome_multipliers, on = ["age", "strains"], 
            how= "left"))
        #self.disease_outcome_multipliers.filter(pl.col("multiplier") != 1)
        
    def external_exposure(self, t, day, P, rng):
        """
        Check for external import of infection.
        """
        if self.varying_external_exposure:
            if day % self.external_exposure_check_period == 0:
                ...
                current_multiplier = self.external_exposure_prob_multipliers[
                        min((day // 364), 
                        (len(self.external_exposure_prob_multipliers) - 1))]
                infected = (
                    susceptibles.filter(
    ...
    def check_vaccines(self, t, day, P, t_per_year, rng):
        ...
        for vaccine, value in self.vaccines.items():
            ...
            if value["years"][0] <= cur_year <= value["years"][1]:
                ...
                if (vaccine in [
                     "ppv23_pcv20_adult_follow", "ppv23_pcv20_adult_follow_2",
                     "ppv23_pcv13_adult_follow", "ppv23_pcv13_adult_follow_2",
                       "ppv23_pcv20_follow", "ppv23_pcv20_follow_2",
                      "ppv23_pcv13_follow", "ppv23_pcv13_follow_2",
                      "ppv23_pcv20_adult_follow_50",
                      "ppv23_pcv20_adult_follow_2_50"]):
                    
                    if (vaccine in ["ppv23_pcv20_follow",
                                    "ppv23_pcv13_follow"]):
                        vacc_target_group = (
                        P.I.filter((pl.col("age")
                      ...
                      (pl.col("vaccines").struct.field("vaccine_type") == \
                            value["previous_vacc"].strip("_final_dose") ) &\
                            ((day - 
              pl.col("vaccines").struct.field("final_vaccine_time")) == (364 +
              (364 * 3 * (vaccine in ["ppv23_pcv20_follow",
                                     "ppv23_pcv13_follow"])) +                                                           
              (364 * 4 * (vaccine in ["ppv23_pcv20_adult_follow_2",
                                     "ppv23_pcv13_adult_follow_2",
                                     "ppv23_pcv20_adult_follow_2_50",
                                    "ppv23_pcv20_follow_2", 
                                    "ppv23_pcv13_follow_2" ]))
              ))).select(["id","age", "age_days", "vaccines", "random"])
                    )
                        #if ((vacc_target_group.height)) | (day > 8736):                                                  
                        #      print("here")                                         
                    else:
              ...
              (364 * 4 * (vaccine in ["ppv23_pcv20_adult_follow_2",
                        ...
                        #if ((vaccine in ["ppv23_pcv20_adult_follow_2",
                        #                 "ppv23_pcv13_adult_follow_2"]) & (
                        #    vacc_target_group.height)) | (day > 10920):
                        #     print("here")
                        #if ((vaccine in ["ppv23_pcv20_adult_follow",
                        #                 "ppv23_pcv13_adult_follow"]) & (
                        #    vacc_target_group.height)) | (day > 8736):
                        #     print("here")
                        #if ((vaccine in ["ppv23_pcv20_follow_2",
                        #                 "ppv23_pcv13_follow_2"]) & (
                        #    vacc_target_group.height)) | (day > 10920):
                        #     print("here")
                    #on time first dose
            ...
            (pl.col("vaccines").struct.field("on_time")).alias("on_time"),
                ...
                elif (vaccine in ["atrisk_ppv23_adult", "atrisk_ppv23_child",
                    ...
                    if (vaccine in ["atrisk_ppv23_adult", 
                        ...
                        if value["previous_vacc"] in ["atrisk_pcv13_adult", 
                                         "atrisk_pcv13_child"]:
                            prev_vaccines = ["atrisk_pcv13_adult", 
                                         "atrisk_pcv13_child"]
                        elif value["previous_vacc"] in ["atrisk_pcv20_adult", 
                                         "atrisk_pcv20_child"]:
                            prev_vaccines = ["atrisk_pcv20_adult", 
                                         "atrisk_pcv20_child"]
                        else:
                            print("error check the vaccines")
                    else:
            ...
            (pl.col("vaccines").struct.field("on_time")).alias("on_time"),
                    ...
                    if (vaccine.endswith(("_1", "_2"))):
                        ...
                        vacc_age_group = (
                            ...
                            #((pl.col("age") > 69) | (pl.col("at_risk")
                    # .is_in(self.at_risk_vaccine_target_groups).is_not()))
                            #make atrisk vacc not eligible
                        (pl.col("vaccines").struct.field("vaccine_type").str
                                 .starts_with(("atrisk")).is_not()) &
                        #make following doses vacc not eligible
                    (pl.col("vaccines").struct.field("vaccine_type")
                             .is_in(["ppv23_pcv20_adult_follow",
                    "ppv23_pcv20_adult_follow_2","ppv23_pcv13_adult_follow", 
                        "ppv23_pcv13_adult_follow_2",
                     "ppv23_pcv20_follow", "ppv23_pcv20_follow_2",
                    "ppv23_pcv13_follow", "ppv23_pcv13_follow_2"]).is_not())
                         ).select(["id","age", "age_days", 
                                        "vaccines", "random"])
                        )
                        """if vacc_age_group.height:
                            print("here")
                            a = (
                                P.I.filter((pl.col("age")
                                    .is_between(value["vaccination_age_range"][0],
            ...
            (pl.col("vaccines").struct.field("on_time")).alias("on_time"),
                ...
                elif (vaccine in ["pcv20_adult_50"#,
                                  #"ppv23_pcv20_adult_follow_50",
                                  #"ppv23_pcv20_adult_follow_2_50"
                                  ]):
                    vacc_age_group = (
                    ...
                    # .is_in(self.at_risk_vaccine_target_groups).is_not()))
                        ...
                        ((pl.col("vaccines").struct.field("vaccine_type")
                                 .is_in(["pcv20_adult",
                                         "ppv23_pcv20_follow"]).is_not()) |
                         ((pl.col("vaccines").struct.field("vaccine_type")
                                  .is_in(["ppv23_pcv20_follow_2"])) &
                          ((day - 
                    ...
                    "ppv23_pcv13_follow", "ppv23_pcv13_follow_2",
                    "ppv23_pcv20_adult_follow_50",
                    "ppv23_pcv20_adult_follow_2_50"]).is_not())
                         ).select(["id","age", "age_days", 
                                        "vaccines", "random"])
                        )
                    if temp_vacc_age_group.height:
                            print("temp_here")
                    """if vacc_age_group.height:
                            print("here")
                            a = (
            ...
            (pl.col("vaccines").struct.field("on_time")).alias("on_time"),
                ...
                elif vaccine.startswith("ppv23_pcv7_follow"):
                    vacc_target_group = (
                    P.I.filter((pl.col("age")
                        .is_between(value["vaccination_age_range"][0],
                                    value["vaccination_age_range"][1])
                      ...
                      ((pl.col("vaccines").struct.field("no_of_doses") == 3) &
                  (pl.col("vaccines").struct.field("vaccine_type") == "pcv7") &
                   (pl.col("vaccines").struct.field("on_time") == 1))
                             )
                             )
                    .select(["id","age", "age_days", "vaccines", "random"])
                    ...
                    if vaccine in ["pcv13_31_final_dose", 
                                   "pcv20_31_final_dose"]:
           ...
           (pl.col("age").is_in(value["vaccination_age_range"][0]).is_not()) |\
                ...
                #print(on_time_first_vacc["vaccines"].struct.schema)
    ...
    def check_exposure(self, t, day, P, rng):
        ...
        if possibly_infected.height:
            ...
            ###AT RISK & ppv23_follows####
            possibly_infected_atrisk_ppv23 = possibly_infected.filter(
                        pl.col("vaccines").struct.field("vaccine_type").is_in(
                            ["atrisk_ppv23_adult",
                             "atrisk_ppv23_child",
                             "atrisk_ppv23_adult_2","atrisk_ppv23_child_2",
                             "ppv23_pcv7_follow","ppv23_pcv20_adult_follow", 
                "ppv23_pcv20_adult_follow_2","ppv23_pcv13_adult_follow", 
                "ppv23_pcv13_adult_follow_2",
                               "ppv23_pcv20_follow", "ppv23_pcv20_follow_2",
                              "ppv23_pcv13_follow", "ppv23_pcv13_follow_2",
         ...
         (possibly_infected_atrisk_ppv23["vaccines"].struct.field("final_vaccine_time") -
            ...
            """possibly_infected = possibly_infected.filter(
                        pl.col("vaccines").struct.field("vaccine_type").is_in(
                            ["atrisk_ppv23_adult_2","atrisk_ppv23_adult",
                             ...
                             "ppv23_pcv7_follow","ppv23_pcv20_adult_follow", 
                "ppv23_pcv20_adult_follow_2","ppv23_pcv13_adult_follow", 
                "ppv23_pcv13_adult_follow_2","ppv23_pcv20_follow", 
                "ppv23_pcv20_follow_2"," ppv23_pcv13_follow", 
                "ppv23_pcv13_follow_2"]).is_not())"""
            ...
            """curr_vaccs = (possibly_infected["vaccines"]
                          .struct.field("vaccine_type").unique())
            if ("pcv13_31_final_dose" in curr_vaccs) | \
                                                ("pcv13_31" in curr_vaccs):
                if possibly_infected.filter(
                    (pl.col("vaccines").struct.field("vaccine_type") == ("pcv13_31")) &
                    (pl.col("vaccines").struct.field("no_of_doses") == (3))).height:
                    print("Here")
                if possibly_infected.filter(
                    (pl.col("vaccines").struct.field("vaccine_type") == ("pcv13_31")) &
                    (pl.col("vaccines").struct.field("no_of_doses") == (4))).height:
                    print("Here")
                    self.vaccine_antibody_df["vaccine_type"].unique().to_list()
                    self.vaccine_antibody_df.filter(
                        pl.col("vaccine_type") == "pcv13_31")["no_of_doses"].unique()
                    myinf = possibly_infected.filter(
                        (pl.col("vaccines").struct.field("vaccine_type") == "pcv13_31") &
                        (pl.col("vaccines").struct.field("no_of_doses") == (4)))
                    myinf.join(
                        self.vaccine_antibody_df, 
                        left_on = [pl.col("vaccines").struct.field("vaccine_type"),
                        pl.col("vaccines").struct.field("no_of_doses"),
    ...
    def check_disease(self,t,day, P, infected, rng):
        ...
        #infected.filter(pl.col("multiplier") != 1)
        ...
        infected_vacc_atrisk_ppv23 = (infected_vacc.drop("meanlog", "sdlog")
            ...
            how="left").drop("vaccine_type","no_of_doses").filter(
                ...
                "ppv23_pcv13_adult_follow_2","ppv23_pcv20_follow", 
                "ppv23_pcv20_follow_2","ppv23_pcv13_follow", 
                "ppv23_pcv13_follow_2","ppv23_pcv20_adult_follow_50",
                "ppv23_pcv20_adult_follow_2_50"])).join(
                    self.prev_vacc_list,
                    left_on = [pl.col("vaccines").struct.field("vaccine_type"),
        ...
        log_antibodies_atrisk_ppv23_ipd = pl.DataFrame([
            ...
            pl.Series("log_antibodies" ,
                      lognorm.ppf(infected_vacc_atrisk_ppv23_ipd["quantile"], 
        ...
        log_antibodies_atrisk_ppv23_ipd = (log_antibodies_atrisk_ppv23_ipd
            ...
            .then( (pl.col("log_antibodies_prev") + 9) * \
                  (pl.col("waning_ratio_prev")) - 9)
        ...
        cur_disease_atrisk_ppv23_ipd = (infected_vacc_atrisk_ppv23_ipd
            ...
            (prob_of_disease_atrisk_ppv23_ipd * 
             pl.col("at_risk_multipliers") * pl.col("multiplier")) 
              )))
        ...
        cur_disease_atrisk_ppv23_cap = (
            cur_disease_atrisk_ppv23_cap.with_columns(
        ...
        infected = infected.with_columns(
                        (pl.lit(rng.rand(infected.height))
                         .alias("random"))).drop("age_coef", 
                                        "meanlog","sdlog", "multiplier")
