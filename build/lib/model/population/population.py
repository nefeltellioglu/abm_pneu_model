...
class Population(object):
    ...
    def introduce_individuals(self, no_inds, ages, age_days, rng):
        ...
        new_inds = pl.DataFrame({
                        ...
                        'age_group': pl.Series(name='age_group', 
                                    values=  [age // 5 for age in ages],
                                        dtype=pl.Int64),
                        'age_days': pl.Series(name='age_days', values= age_days,
                                              dtype=pl.Int64),
                        'random': pl.Series(name='random', 
                                values= [rng.random() for _ in range(no_inds)]),
                        })
    ...
    def introduce_births_and_migrations(self, t, t_per_year, no_of_births, 
                                        no_of_migrations, age_dist_mig,rng):
        """
        ...
        #new births
        ages = [0] * no_of_births
        age_days = [t * t_per_year % 364] * no_of_births
        self.I, self.next_id = self.introduce_individuals(no_of_births, 
                                                ages, age_days, rng)
        
        #migrated population
        mig_ages, mig_age_days = gen_ages(t_per_year, no_of_migrations, age_dist_mig, rng)
        self.I, self.next_id = self.introduce_individuals(no_of_migrations, 
                                                mig_ages, mig_age_days, rng)
