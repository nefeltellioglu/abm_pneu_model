...
class Simulation(object):
    ...
    def create_population(self):
        ...
        self.P = gen_age_structured_pop(self.p['t_per_year'], self.P, 
                                self.p['pop_size'], self.age_dist, self.rng)
    ...
    def _load_demographic_data(self):
        ...
        initial_death_rates = self._parse_age_rates(os.path.join(
                self.p['resource_prefix'],
                self.p['initial_death_rates']), annual_factor, True)
        
        if self.p['later_death_rates']:
            
            later_death_rates = self._parse_age_rates(os.path.join(
                    self.p['resource_prefix'],
                    self.p['later_death_rates']), annual_factor, True)
            
            
            
            self.death_rates = [initial_death_rates] *\
                self.p['year_to_change_death_rates'] * self.p['t_per_year'] +\
                [later_death_rates] *\
                    ((self.p['years'][1] -self.p['years'][0]) -\
                     self.p['year_to_change_death_rates']) *\
                    (self.p['t_per_year'] + 1)
    ...
    def update_all_demo(self, t):
        ...
        death_rates = pl.Series("death_rates", [np.array([i[0] for i in self.death_rates[t].values()])])
