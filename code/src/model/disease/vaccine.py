...
def add_antibody_levels_v116():
    ...
    #serotype ┆ group        ┆ immune_group ┆ meanlog  ┆ sdlog    ┆ dist      ┆ schedule ┆ vaccine_type
...
def waning_ratio(vaccine_type, day, final_vaccine_time,
    ...
    halflife = halflife.with_columns(
        ...
        .otherwise(np.log(0.5)/waning_halflife_day_child).alias("daily_decay")
            ...
            np.exp(pl.col("daily_decay") * (day - pl.col("final_vaccine_time"))).alias("waning_ratio_2")
            )
...
def waning_ratio_extended(vaccine_type, day, final_vaccine_time,
    ...
    """
    #if halflife.filter((pl.col("age")== 0) & (pl.col("age_days") < 56)).height:
    #    print("check maternal waning")
    #    initial_day = day
    #    day = day + 28
...
def remove_v116_maternal(popDf, vaccine, value, t, day, t_per_year, vaccines, 
    ...
    #    print("here: check if v116 maternal is removed correctly")
...
def roll_v116_maternal(P, vaccine, value, t, day, t_per_year, vaccines, 
    ...
    #if late_first_vacc.height:
    #    print("check why there is late vaccination")
    #if on_time_first_vacc.filter((pl.col("age") != 0) ).height:
    #    print("check why older age groups receive mat vacc")
    #if on_time_first_vacc.filter(pl.col("age_days") >= period).height:
    #    print("check why older age groups receive mat vacc")
