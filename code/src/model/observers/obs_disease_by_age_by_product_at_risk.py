...
class DiseaseObserverByAgeByProduct(Observer):
    def __init__(self, h5file, vaccines, interval, data_directory):
        ...
        desc = {'t': tb.UInt32Col(pos=0),
                ...
                'when_vaccine_delivered': tb.StringCol(pos=4, itemsize = 20),
                'inds_per_age_group': tb.UInt32Col(pos=5,
                                                  shape=(self.no_age_groups)), 
                'serotype_group': tb.StringCol(pos=6, itemsize = 20),
                
        
        }
        
        strain_fname = os.path.join(data_directory,'disease/strain_list.dat')
        strain_list = []
        for l in open(strain_fname, 'r'):
            if l[0] == '#': continue
            line = l.strip().split(' ')
            strain_list.append(line[0])
        self.strain_list = list(np.sort(strain_list))
        ...
        pcv20_strains = self.pcv15_strains.append(self.pcv20_not_pcv15).to_list()
        
        self.serotype_groups = { 
            "20vPCV": pcv20_strains,
            "V116": self.v116_strains.to_list(),
            "V116_only": self.v116_only_strains.to_list(),
            "NVT": list(set(self.strain_list) - set(self.vt_strains.to_list())),
            "20vPCV_only": list(set(pcv20_strains) - set(self.v116_strains.to_list())),
            "All": self.strain_list,
            "All_no": []
            }
        super(DiseaseObserverByAgeByProduct, self).__init__(h5file =h5file, \
                            label = 'disease_byagebyproduct', description = desc,
                            title = 'DiseaseObserverByAgeByProduct')
            ...
            for cur_vaccine in delivered_vaccines:
                for individuals, at_risk_values in self.individual_types.items():
                    for vacc_delivered, time_since_vacc in self.vaccine_delivered.items():
                        for serotype_group_name, cur_serotype_group in self.serotype_groups.items():
                            """
                            cur_vaccine = "all_no"
                            ...
                            cur_serotype_group = self.v116_strains.to_list()
                            self.strain_list + []
                            """
                            if (cur_vaccine == "all_no") & (time_since_vacc < 999999):
                                continue
                            elif cur_vaccine == "all_no":
                                cur_pop = (pop.I.select("at_risk", "age", "age_days", "vaccines", "strain_list")
                                               .unnest("vaccines")
                            ...
                            if serotype_group_name != "All_no":
                                cur_pop = cur_pop.filter((pl.col("strain_list")
                                                .list.eval(pl.element()
                                                .is_in(cur_serotype_group))
                                                .list.any()
                                                ))
                            ...
                            self.row['when_vaccine_delivered'] = vacc_delivered
                            self.row['serotype_group'] = serotype_group_name
                        ...
                        for vacc_delivered, time_since_vacc in self.vaccine_delivered.items():
                            for serotype_group_name, cur_serotype_group in self.serotype_groups.items():
                                ...
                                if (cur_vaccine == "all_no") & (time_since_vacc < 999999):
                                    ...
                                    cur_pop = (pop.disease_pop.select("at_risk", "age", "age_days", "vaccines", "disease", "strain_list")
                                                   .unnest("vaccines")
                                               .filter((pl.col("at_risk").is_in(at_risk_values)) &
