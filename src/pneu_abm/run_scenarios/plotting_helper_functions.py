...
def get_prev_data(directory,data_dir, filename, year, indigenous = None):
    ...
    strain_list = strains["serotype"].to_list()
    ...
    for filename in ind_file_addresses:#[vaccination_file_addresses[0],vaccination_file_addresses[1]]:
        try:
            ...
            with h5py.File(filename, "r") as f:
                ...
                # create Polars DataFrame
                df = pl.DataFrame(data_dict)
                #pl.from_numpy(f[group_name][key][()],
                #    schema=f[group_name][key][()].dtype.names, orient="col")
...
def plot_base_prev(axes, data_dir,
                   df, strain_list, vaccines, strains, years = None,ymax = 1.5,
                   pcv_strains = False,
                   overall_prev = False, plot_vaccine_lines = True, 
                   indigenous = None, df_overall = pl.DataFrame(), t_per_year = 52):
    ...
    if years:
        ...
        weeks = weeks[:((years[1] - start_year) * t_per_year)]
        total_years = years[1] - years[0]
        #times = [(years[0] + time / t_per_year) for time in weeks]
        times = [(start_year + time / t_per_year) for time in df["t"].unique().sort()]
        #interval = ((t_per_year * (years[0] - 2002)),
        #                       ((years[1] - 2002) * t_per_year))
    ...
    if plot_vaccine_lines:
        if indigenous:
            if 2001 in range(years[0], years[1]):
                ...
                plt.text(2001.1,ymax*0.5,'7vPCV\nschedule', color ='#7f18f5', 
                     fontsize=9)
    ...
    for s,strain in enumerate(df_agg["strain_names"].unique().sort()):
        if (strain in sorted(set(strains.filter(pl.col("7vPCV"))['serotype']))) or (strain == "7vPCV"):
            if pcv_strains:
                ...
                color = colors[0]
                label = "7vPCV"
            if (strain in sorted(set(strains.filter(pl.col("7vPCV"))['serotype']))):
                alpha = 1
                alpha_fill = 0.05
            else:
                alpha = 1
                alpha_fill = 0.5
        elif (strain in sorted(set(strains.filter(pl.col("13vPCV"))['serotype']))) or \
                    (strain == "13vPCV"):
            if pcv_strains:
                #color = colors[c_i]
            ...
            if (strain in sorted(set(strains.filter(pl.col("13vPCV"))['serotype']))):
                alpha = 1
                alpha_fill = 0.05
            else:
                alpha = 1
                alpha_fill = 0.5
        elif (strain in sorted(set(strains.filter(pl.col("23vPPV"))['serotype']))) or \
            (strain == "23vPPV"):
            color = colors[-1]
            label = "23vPPV (not 13vPCV)"
            if (strain in sorted(set(strains.filter(pl.col("23vPPV"))['serotype']))):
                alpha = 1
                alpha_fill = 0.05
            else:
                alpha = 1
