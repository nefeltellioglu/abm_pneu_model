...
class DiseaseObserverByVaccine(Observer):
    def __init__(self, h5file, vaccines, interval, strains_df):
        ...
        desc = {'t': tb.UInt32Col(pos=0),
                ...
                'cap_per_100k_per_vaccines': tb.Float64Col(pos=2,
                                              shape=(self.no_vaccines)), 
                'ipd_per_100k_per_vaccines': tb.Float64Col(pos=3,
                                              shape=(self.no_vaccines)),
                'no_of_cap_per_vaccines':tb.UInt32Col(pos=4,
                                              shape=(self.no_vaccines)),
                'no_of_ipd_per_vaccines': tb.UInt32Col(pos=5,
                                              shape=(self.no_vaccines)),
                'vaccine_names': tb.StringCol(pos=6,
                                              itemsize = self.vacc_name_length,
                                              shape=(self.no_vaccines)),
                'cap_per_100k_per_vaccines_last5_years': tb.Float64Col(pos=7,
                                              shape=(self.no_vaccines)), 
                'ipd_per_100k_per_vaccines_last5_years': tb.Float64Col(pos=8,
                                              shape=(self.no_vaccines)),
                'no_of_cap_per_vaccines_last5_years':tb.UInt32Col(pos=9,
                                              shape=(self.no_vaccines)),
                'no_of_ipd_per_vaccines_last5_years': tb.UInt32Col(pos=10,
                                              shape=(self.no_vaccines)),
                'cap_per_100k_per_vaccines_last1_years': tb.Float64Col(pos=11,
                                              shape=(self.no_vaccines)), 
                'ipd_per_100k_per_vaccines_last1_years': tb.Float64Col(pos=12,
                                              shape=(self.no_vaccines)),
                'no_of_cap_per_vaccines_last1_years':tb.UInt32Col(pos=13,
                                              shape=(self.no_vaccines)),
                'no_of_ipd_per_vaccines_last1_years': tb.UInt32Col(pos=14,
                                              shape=(self.no_vaccines)),
                'no_of_cap_per_vaccines_per_age_group':tb.UInt32Col(pos=15,
                              shape=(self.no_age_groups, self.no_vaccines)),
                'no_of_ipd_per_vaccines_per_age_group': tb.UInt32Col(pos=16,
                              shape=(self.no_age_groups, self.no_vaccines)),
                'no_of_inds_per_age_group':tb.UInt32Col(pos=17,
                              shape=(self.no_age_groups, self.no_vaccines)),
                ...
                'no_of_ipd_per_vaccines_per_age_group_last5_years': \
                    tb.UInt32Col(pos=19,
                              shape=(self.no_age_groups, self.no_vaccines)),
                'no_of_inds_per_age_group_last5_years':tb.UInt32Col(pos=20,
                              shape=(self.no_age_groups, self.no_vaccines)),
        ...
        super(DiseaseObserverByVaccine, self).__init__(h5file =h5file, \
                            label = 'disease_byvaccine', description = desc,
                            title = 'DiseaseObserverByVaccine')
    ...
    def update(self, t, pop, **kwargs):
        ...
        if t % self.interval == self.observed_day:  
            ...
            for run_no in run_nos:
                ...
                disease_pop1 = cur_pop_disease.unnest("vaccines").with_columns(
                ...
                disease_counts_per_vaccine = disease_counts_per_vaccine.sort(
                    pl.col("disease"),
                    pl.col("vaccine_type").replace(vaccine_order))
                ...
                disease_counts_per_vaccine_df_last5_years = pl.DataFrame([
                   ...
                   pl.Series("vaccine_type", vaccine_names * 2),
                    pl.Series("count", [0] * len(vaccine_names) * 2),
                        ...
                        pl.col("final_vaccine_time")/ (365 // self.interval) + \
                            self.interval * 5 >= t)
                ...
                cap_age_groups = pl.DataFrame([
                    ...
                    pl.Series("count", 
                      ...
                      on = ["dis_age_groups", "vaccine_type"], how= "left" ).sort(
                          pl.col("dis_age_groups").replace(self.age_groups_order), 
                          pl.col("vaccine_type").replace(vaccine_order)
                          ).group_by("dis_age_groups").agg("count").sort(
                                 pl.col("dis_age_groups")
                                 .replace(self.age_groups_order)
                                 )["count"].to_list()
                ...
                self.row['cap_per_100k_per_vaccines'] =   \
                    ...
                    no_vacc_given_all_pop.sort(pl.col("vaccine_type").replace(
