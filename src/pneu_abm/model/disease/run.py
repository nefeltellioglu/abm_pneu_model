...
def go_single(p, disease_type, cmatrix, sim_type=DisSimulation, 
                          verbose=False):
    """
    ...
    if os.path.isfile(disease_fname) and not p['overwrite']:
        ...
        disease = None
        try:
            disease = disease_type(p, cmatrix, disease_fname, mode='r')
    ...
    disease = disease_type(p, cmatrix, disease_fname, mode='w')
    sim = sim_type(p, disease)
